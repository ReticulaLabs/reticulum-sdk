...
use crate::iface::{
    decode_rx, encode_tx, rx_frame_capacity, set_tcp_sockopts, spawn_tx_drain_task,
    tx_frame_buffer_sizes, CONNECT_TIMEOUT, IfacConfig, Interface, InterfaceContext, InterfaceMode,
    INITIAL_RECONNECT_BACKOFF, MAX_RECONNECT_BACKOFF, RxMessage, configured_bitrate,
};
...
impl BackboneClient {
    ...
    pub async fn spawn(context: InterfaceContext<BackboneClient>) {
        ...
        // Size the TX/RX buffers from the connection's negotiated MTU so
        // they can carry frames of every possible size up to that MTU
        // (including worst-case HDLC escaping) without the fixed 512 KiB
        // allocation that was previously made regardless of the actual MTU.
