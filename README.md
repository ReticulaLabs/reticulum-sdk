...
| `lora-linux` | yes\* | Linux backends for the LoRa interface (spidev ioctls + `gpio-cdev` GPIO). Implies `lora`. Linux-only; leave off for embedded builds. \*Enabled by default only on Linux; it is inert on other platforms. |
